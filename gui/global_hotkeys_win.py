...
def modifiers_mask_from_config(gs: dict) -> int:
    """Zwei Modifikatoren (modifier_1, modifier_2) zu RegisterHotKey-Flags."""
...
def _modifier_physically_held(mod_name: str) -> bool:
    name = str(mod_name or "none").strip().lower()
    if name in ("none", "", "-"):
        return True
    vk = _VK_BY_MOD.get(name)
...
def hotkey_binding_physically_held(
    gs: dict, key_field: str, default_key: str = "Y"
) -> bool:
    """True wenn Modifikatoren und Taste laut Einstellung physisch gedrückt sind."""
    m1 = str(gs.get("modifier_1", "control") or "control").strip().lower()
    m2 = str(gs.get("modifier_2", "shift") or "shift").strip().lower()
    if not _modifier_physically_held(m1):
        return False
    if not _modifier_physically_held(m2):
        return False
    key = str(gs.get(key_field, default_key) or default_key)
    vk = vk_from_key_spec(key)
...
def vk_from_key_spec(spec: str) -> int:
    """Buchstabe, Ziffer oder Token → Virtual-Key für RegisterHotKey."""
...
class GlobalHotkeyController:
    """Registriert Hotkeys am HWND des Hauptfensters."""
    ...
    def apply_config(self, gs: dict) -> None:
        """``GlobalShortcutsSettings``-Dict lesen und Hotkeys neu anlegen."""
        self.unregister_all()
        if sys.platform != "win32" or not self._user32:
            return
        ...
        def add(action: str, key_field: str, default: str) -> None:
            key = str(gs.get(key_field, default) or default)
            specs.append((action, mods_all, vk_from_key_spec(key)))

        add("contest_play", "key_contest_play", "P")
        add("live_ptt_latch", "key_live_ptt_latch", "X")
        add("live_ptt_momentary", "key_live_ptt_momentary", "Y")
